...
list_of_packages <- c("bookdown", "ggplot2", "ggplot2movies", "qqman", "ggforestplot", "cowplot", "ggrepel", "dplyr")
...
These pages go through the uses of different scripts. We use [simulated data](#simple_data) that epidemiology researchers are likely to have access to, as well as [larger data](#movies) and [genome-wide association data](#gwas_data), and a [correlation matrix](#matrix_corr) researchers may come across during their work. You can download the `.Rmd` file from which this page is built by clicking the 'Code' tab at the top of the page.

\  

First, we need some simulated data to use for the plots. Additionally we can spice up the plots we make from any of our scripts by making our own colour choices and deciding what we want the plot style to be like.

\  

# Data {#data .tabset}

## Basic data {#simple_data}
...
```{r simple_data, eval=TRUE, include=TRUE, echo=TRUE, error=FALSE, message=FALSE, warning=FALSE, cache=TRUE, fig.align='center', comment=""}
...
For a larger and more comprehensive data set the [`ggplot2`](https://ggplot2.tidyverse.org/) package has an extended package ([`ggplot2movies`](https://cran.r-project.org/web/packages/ggplot2movies/)) that contains a large data set of data from IMDB which we can use. The data is of thousands of films (`title`) and data on those films including a top level grouping (`Genre`) and  `year`, `length`, `budget`, `rating` and number of `votes`.
```{r movies, eval=TRUE, include=TRUE, echo=TRUE, error=FALSE, message=FALSE, warning=FALSE, cache=TRUE, fig.align='center', comment=""}
##### Packages
library(ggplot2)
...
For GWAS data the [`qqman`](https://cran.r-project.org/web/packages/qqman/vignettes/qqman.html) package has a dummy GWAS data set we can use. The data is not as large as a full GWAS data set, but for testing plots it provides enough information 
```{r gwas_data, eval=TRUE, include=TRUE, echo=TRUE, error=FALSE, message=FALSE, warning=FALSE, cache=TRUE, fig.align='center', comment=""}
library(qqman)
...
```{r matrix_corr, eval=TRUE, include=TRUE, echo=TRUE, error=FALSE, message=FALSE, warning=FALSE, cache=TRUE, fig.align='center', comment=""}
...
I use a custom `ggplot` theme for my plots because I don't like the themes `ggplot` uses. The code for my theme is at the end of the document in the [my_theme](#my_theme) section. You can adapt this, [start from scratch](https://ggplot2.tidyverse.org/reference/theme.html), edit specific aspects of the theme with the `theme()` functon, or use a preset theme from `ggplot` such as `theme_bw()`.  

```{r my_theme, eval=TRUE, include=FALSE, echo=FALSE, error=FALSE, message=FALSE, warning=FALSE, cache=TRUE, fig.align='center', comment=""}
my_theme <- function () 
  { 
  ggplot2::theme(
...
In addition, I use a custom discrete and continuosu colour pallete using the [`wesanderson`](https://github.com/karthik/wesanderson) and [`yarrr`](https://bookdown.org/ndphillips/YaRrr/jumpin.html) packages.

```{r palette, eval=TRUE, include=TRUE, echo=TRUE, error=FALSE, message=FALSE, warning=FALSE, cache=TRUE, fig.align='center', comment=""}
##### Packages
library("wesanderson")
library("yarrr")
##### Palettes
...
# Hidden code {#hidden_code .tabset}

## My theme {#my_theme}  
```{r ref.label="my_theme", results='hide', echo = TRUE}
```
